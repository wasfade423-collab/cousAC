...
class Universite extends DbConnect
{
    ...
    {
        try {
            $stmt = $this->dbConnect->prepare("SELECT universite_id, descriptions, nom, ville, numero_telephone, email FROM universites");
            if ($stmt->execute()) {
    ...
    {
        try {
            $stmt = $this->dbConnect->prepare("SELECT * FROM universites WHERE universite_id =:id");
